    ...
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-12 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('gym_admin.equipment.create') }}" class="btn btn-primary">Add Equipment</a>
                @if (session('success'))
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="ms-2">
                                                    <a href="{{ route('gym_admin.equipment.show', encrypt($item->id)) }}"
                                        ...
                                        <td>
                                            <span
                                                {{-- class="badge bg-label-{{ $item->condition === 'New' ? 'success' : ($item->condition === 'Needs Maintenance' ? 'Poor' : 'warning') }} me-1"> --}}
                                                class="badge bg-{{ $item->condition === 'New' ? 'success' : ($item->condition === 'Good' ? 'primary' : ($item->condition === 'Poor' ? 'warning' : 'success')) }} me-1">
                                        ...
                                        <td>
                                            <div class="d-inline-block text-nowrap">
                                                <!-- Edit Button -->
                                                <a href="{{ route('gym_admin.equipment.edit', encrypt($item->id)) }}"
                                                    class="btn btn-sm btn-icon"><i class="bx bx-edit-alt me-1"></i></a>

                                                <!-- Delete Button -->
                                                <form
                                                    action="{{ route('gym_admin.equipment.destroy', encrypt($item->id)) }}"
                                                    method="POST" class="d-inline" id="deleteform_{{ $item->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-icon"
                                                        onclick="if (confirm('Delete?')) { document.getElementById('deleteform_{{ $item->id }}').submit(); }">
                                                ...
                                                <div class="dropdown d-inline">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            href="{{ route('gym_admin.equipment.show', encrypt($item->id)) }}">
